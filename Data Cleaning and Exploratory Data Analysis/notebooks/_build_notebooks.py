...
collider_cells = (
    ...
    + [
        ...
        md(
            ...
            "For AI this is a training-data problem. A model trained on "
            "approved loans has never seen the applicants the old policy "
            "rejected, so it inherits that policy's blind spots — and it "
            "can learn manufactured trade-offs exactly like the ability–"
            "portfolio one above."
        ),
        md(
            "## What to do about it\n\n"
            "Three concrete steps, in order:\n\n"
            "1. **Name the doorway.** Before you trust any relationship, "
            "ask what had to happen for a row to exist, and look for a "
            "column that records it — here `data_source` and `admitted` "
            "do exactly that. If no such column exists, ask whoever "
            "produced the data how the rows were collected. A dataset "
            "with *no visible* selection is the one to worry about most, "
            "because the doorway is still there — you just cannot see it.\n\n"
            "2. **Measure on the full population, not the survivors.** "
            "This file still contains the entire applicant pool, so "
            "compute the ability–portfolio relationship on *that* "
            "(`corr` ≈ 0), not inside the admitted subset (`corr` ≈ "
            "−0.35). The rule: never filter down to the selected group "
            "before measuring a relationship between the variables that "
            "drove the selection — that filter is what manufactures the "
            "pattern. For a model, train on the pre-selection rows so it "
            "sees the applicants the old policy rejected.\n\n"
            "3. **If the rejected rows are genuinely gone, scope the "
            "claim.** When all you have is the selected file, you cannot "
            "generalize past it. Say so out loud — 'among admitted "
            "students…' — instead of implying the pattern holds for "
            "everyone, and flag that a model built on it will reproduce "
            "the original selection policy, including whoever it used to "
            "reject."
        ),
        md(
            "## Takeaway\n\n"
